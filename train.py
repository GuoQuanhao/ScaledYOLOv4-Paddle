...
def train(hyp, opt, tb_writer=None):
    ...
    print(logdir, 'logdir', tb_writer.logdir)
    ...
    for epoch in range(start_epoch, 10):  # epoch ------------------------------------------------------------------
        model.train()
    ...
    for f1, f2 in zip([wdir + 'last.pdparams', wdir + 'best.pdparams', 'results.txt'], [flast, fbest, fresults]):
        if os.path.exists(f1):
            ...
            ispt = f2.endswith('.pdparams')  # is *.pdparams
            strip_optimizer(f2, f2.replace('.pdparams','_strip.pdparams')) if ispt else None  # strip optimizer
            os.system('gsutil cp %s gs://%s/weights' % (f2, opt.bucket)) if opt.bucket and ispt else None  # upload
    # Finish
    if not opt.evolve:
        plot_results(save_dir=logdir)  # save as results.png
