...
def build_targets(p, targets, model):
    ...
    ai = np.tile(np.arange(na).astype('float32').reshape(na, 1), [1, nt])  # same as .repeat_interleave(nt)
    targets = np.concatenate((np.tile(targets, [na, 1, 1]), ai[:, :, None]), 2)  # append anchor indices

    g = 0.5  # bias
    off = np.array([[0, 0],
                   [1, 0], [0, 1], [-1, 0], [0, -1],  # j,k,l,m
                  ]).astype('float32') * g  # offsets

    for i in range(det.nl):
        anchors = det.anchors[i].numpy()
        gain[2:6] = np.array(p[i].shape, dtype='float32')[[3, 2, 3, 2]]  # xyxy gain
        ...
        if nt:
            ...
            # t = t.masked_select(j.unsqueeze(-1).tile([t.shape[-1]])).reshape([-1, t.shape[-1]])  # filter # 偶尔报错
            t = t[j]
            # Offsets
            gxy = t[:, 2:4]  # grid xy
            gxi = gain[[2,3]] - gxy
            j, k = ((gxy % 1. < g) & (gxy > 1.)).astype('float32').T
            l, m = ((gxi % 1. < g) & (gxi > 1.)).astype('float32').T
            j = np.stack((np.ones_like(j), j, k, l, m))
            t = np.tile(t, [5, 1, 1])[j.astype(bool)]
            offsets = (np.zeros_like(gxy)[None] + off[:, None])[j.astype(bool)]
        else:
            t = targets[0]
            offsets = 0
        ...
        indices.append((paddle.to_tensor(b), paddle.to_tensor(a), paddle.to_tensor(gj.clip(0, gain[3]), dtype='int64'), paddle.to_tensor(gi.clip(0, gain[2]), dtype='int64')))  # image, anchor, grid indices
        gj, gi = gj.clip(0, gain[3]), gi.clip(0, gain[2])
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            x = paddle.concat((box[i], paddle.to_tensor(x.numpy()[i.numpy(), j.numpy() + 5, None]), paddle.to_tensor(j.numpy()[:, None]).astype('float32')), 1)
